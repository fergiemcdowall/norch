...
// var superrequest = supertest(url)
...
describe('Can I Index Data?', function () {
  ...
  it('should post and index a file of data', function (done) {
    fs.createReadStream('./node_modules/reuters-21578-json/data/fullFileStream/justTen.str')
      ...
      .on('data', function(data) {
        data.toString().should.be.exactly('"batch indexed"')
      })    
      .on('error', function(error) {
        false.should.be.exactly(true)  // error
        return done()
      })    
      .on('end', function() {
        return done()
  ...
  it('search should return 12 docs', function (done) {
    var i = 0
    request(url + '/search')
      .on('data', function(data) {
        i++
      ...
      .on('end', function(data) {
        i.should.be.exactly(12)
        return done()
      })
  })

  it('should say that there are now 12 documents in the index', function (done) {
    request(url + '/docCount')
      .on('data', function(data) {
        data.toString().should.be.exactly('"12"')
      })
      .on('error', function(error) {
        false.should.be.exactly(true)  // error
        return done()
  ...
  //   request(url + '/tellmeaboutmynorch/lastUpdate')
  //     .on('data', function(data) {
...
describe('Can I do indexing and restore?', function () {
  ...
  describe('Making a backup', function () {
    ...
    it('should generate a backup file', function (done) {
      request.get(url + '/snapshot')
        .on('data', function(data) {
          data.toString().should.be.exactly('"replication complete"')
        })    
        .on('error', function(error) {
          false.should.be.exactly(true)  // error
          return done()
  ...
  describe('Restoring from a backup', function () {
    ...
    it('should get latest snapshot and import in into a new norch', function (done) {
    ...
    it('should be able to return 12 results from the replicant norch', function (done) {
      var i = 0
      var q = {    
        query: {
          AND: {'*': ['*']}
...
describe('Can I empty an index?', function () {
  ...
  it('should empty the index', function (done) {
    request.del(url + '/flush')
      .on('data', function(data) {
        data.toString().should.be.exactly('"index flushed"')
      })
      .on('error', function(error) {
        false.should.be.exactly(true)  // error
        return done()
...
describe('Can I Index and search in bigger data files?', function () {
  ...
  it('should initialize a norch server', function (done) {
    require('../lib/norch.js')({
      indexPath: sandbox + 'bigger-norch',
      port: 5050
    }, function (err, norch) {
      if (err) false.should.eql(true)
      done()
...
// uncomment this when running tests on empty dir

  // it('should post and index a file of data that can be filtered on', function (done) {
  ...
  it('should say that there are now 1000 documents in the index', function (done) {
    request(url + '/docCount')
      .on('data', function(data) {
        data.toString().should.be.exactly('"1000"')
      })
      .on('error', function(error) {
        false.should.be.exactly(true)  // error
        return done()
