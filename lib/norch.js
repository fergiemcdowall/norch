...
app.set('hostname', program.hostname);
...
if (process.argv.indexOf('-h') == -1) {
  http.createServer(app).listen(app.get('port'), app.get('hostname'), function(){
    ...
    console.log('http://fergiemcdowall.github.io/Norch'.red + '  \\/__/'.red);
    console.log();
    console.log('Norch server listening on hostname ' + app.get('hostname') + ' on port ' + app.get('port'));
    console.log();
  });  
}
