module.exports = function (ops, callback) {
  ...
  if (process.argv.indexOf('-h') == -1) {
    http.createServer(app).listen(app.get('port'),app.get('hostname'), function () {
      ...
      if (callBackRequested)
        return callback('started ' + app.get('port'));
    });
  }
  ...
  app.post('/login', function (req, res) {
    var file = fs.readFileSync('./auth.json','utf-8');
    var data = JSON.parse(file);
    var reqData = req.body.document;
    if (reqData.username == data.username && reqData.password == data.password) {
